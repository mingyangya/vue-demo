<template>
  <div
    ...
    <!-- 画中画开启时蒙层 -->
    <div :class="['gkplayer-iconfont', $style.pictureInPictureMask]" v-if="isPictureInPicture"></div>

    <div :class="[$style.title]" player-title v-if="title">当前播放: {{title}}</div>

    <slot name="cover"></slot>

    <Loading v-if="loading"></Loading>
    <div :class="$style.bigPlayBtn" player-bigplaybtn v-if="paused && isShowBigPlayBtn" @click.stop="play">
      <div :class="['gkplayer-iconfont', $style.bigPlayIcon]" player-bigplaybtn-icon></div>
    </div>

    <!-- 字幕, 这里为了实现外描边的效果，放了两层文字 -->
    <div :class="[$style.subtitle, $style.subtitleStroke, {[$style.subtitleStrokeFullScreen]: isFullScreenStatus}]" v-show="subtitle && !isPictureInPicture">{{subtitle}}</div>
    <div :class="[$style.subtitle, {[$style.subtitleStrokeFullScreen]: isFullScreenStatus}]" v-show="subtitle && !isPictureInPicture">{{subtitle}}</div>

    <!-- 工具条 -->
    <div :class="$style.controlMask" @click.stop></div>
    <div :class="[$style.controlWrap]" player-control-wrap @click.stop>
      <div :class="$style.left">
        <template v-if="paused">
          <div :class="['gkplayer-iconfont', $style.icon, $style.playIcon]" player-control player-play @click.stop="play"></div>
        </template>
        <template v-else>
          <div :class="['gkplayer-iconfont', $style.icon, $style.pauseIcon]" player-control player-pause @click.stop="pause"></div>
        </template>
        <div :class="[$style.thumbTimeWap]" player-thumb-time-wap v-if="isMobile">{{thumbTime}}</div>
        <div :class="['gkplayer-iconfont', $style.icon,, $style.prevIcon]" player-prev v-if="showPrev" @click.stop="prev"></div>
        <div :class="['gkplayer-iconfont', $style.icon, $style.nextIcon]" player-next v-if="showNext" @click.stop="next"></div>
        <div :class="$style.statusTime" player-status-time v-if="!isMobile">{{thumbTime}}<span :class="$style.duration"> / {{durationTime}}</span></div>
      </div>
      <div :class="$style.right">
        <div :class="[$style.durationTimeWap]" player-duration-time-wap v-if="isMobile">{{durationTime}}</div>
        <!-- 清晰度 -->
        <div :class="$style.quality" player-quality v-if="!isMobile">
          <div :class="$style.current" player-quality-cur v-if="quality">
            <span @click.stop="openQualityList">{{quality.name}}</span>
          </div>
          <ul :class="[$style.qualityList, {[$style.on]: isShowQualityList}]" player-quality-list>
        ...
        <!-- 播放速率 -->
        <div :class="$style.rate" player-rate v-if="rateEnabled">
          <div :class="$style.current" player-rate-cur>
            <span @click.stop="openRateList">{{rate}}x</span>
          </div>
          <ul :class="[$style.rateList, {[$style.on]: isShowRateList}]" player-rate-list>
        ...
        <template v-if="pictureInPictureEnabled">
          <template v-if="isPictureInPicture">
            <div :class="['gkplayer-iconfont', $style.icon, $style.pictureInPictureOffBtn]" player-webfullscreen player-webfullscreen-on @click.stop="closePictureInPicture">
              <div :class="$style.tooltips">退出画中画</div>
            </div>
          </template>
          <template v-else>
            <div :class="['gkplayer-iconfont', $style.icon, $style.pictureInPictureOnBtn]" player-webfullscreen player-webfullscreen-off @click.stop="openPictureInPicture">
              <div :class="$style.tooltips">开启画中画</div>
            </div>
          </template>
        </template>
        <!-- 音量 -->
        <div :class='$style.volumeWrap' v-show="volumeEnabled">
          <div :class="['gkplayer-iconfont', $style.icon, $style.volumeBtn]" player-webfullscreen player-webfullscreen-on @click.stop="toggleVolume">
            <div :class="$style.tooltips" v-show="!isShowVolumeBar">音量</div>
          </div>
          <div :class="$style.volumeBarWrap" v-show="isShowVolumeBar" @click.stop="volumeBarClick" @mousedown="volumeDragStart" ref="volumeBarWrap">
            <div :class="$style.volumeBar">
              <div :class="$style.volumeBarProgress" :style="{'height': `${volume * 100}%`}">
                <div :class="$style.volumeBarProgressDot" />
              </div>
            </div>
          </div>
        </div>
        ...
        <template v-if="webFullScreenEnabled && !isFullScreenStatus">
          <template v-if="isWebFullScreen">
            <div :class="['gkplayer-iconfont', $style.icon, $style.webFullscreenOnBtn]" player-webfullscreen player-webfullscreen-on @click.stop="toggleWebFullScreen">
              <div :class="$style.tooltips">退出网页全屏</div>
            </div>
          </template>
          <template v-else>
            <div :class="['gkplayer-iconfont', $style.icon, $style.webFullscreenOffBtn]" player-webfullscreen player-webfullscreen-off @click.stop="toggleWebFullScreen">
              <div :class="$style.tooltips">网页全屏</div>
            </div>
          </template>
        </template>
        <!-- 全屏 -->
        <template v-if="isFullScreenStatus">
          <div :class="['gkplayer-iconfont', $style.icon, $style.fullscreenOnBtn]" player-fullscreen player-fullscreen-on @click.stop="toggleFullScreen">
            <div :class="$style.tooltips">退出全屏</div>
          </div>
        </template>
        <template v-else>
          <div :class="['gkplayer-iconfont', $style.icon, $style.fullscreenOffBtn]" player-fullscreen player-fullscreen-off @click.stop="toggleFullScreen">
            <div :class="$style.tooltips">全屏</div>
          </div>
        </template>
      </div>
      <div :class="$style.barWrap" player-bar-wrap ref="progressBarWrap">
        <div :class="[$style.barTime, {[$style.on]: isShowTouchTime}]" player-bar-time ref="touchTime">{{touchTime}}</div>
        <div :class="$style.bar" player-bar>
          ...
          <div :class="$style.loaded" player-bar-loaded :style="{width: loadedPercentage }"></div>
          <div :class="$style.played" player-bar-played ref="played" :style="{width: thumbPercentage }">
            <span :class="$style.tooltip" player-bar-tooltip :player-is-ended="isEnded"></span>
          </div>
        </div>
      </div>
    </div>
    ...
    <template v-if="isShowQualityTip">
      <div :class="$style.qualityTip" player-quality-tip>
...
<script>
import VideoPlayer from './NativeVideo'
import EncryptVideoPlayer from './AliyunVideo'
...
export default {
  ...
  computed: {
    ...
    pictureInPictureEnabled () {
      return this.config.enablePictureInPicture && document.pictureInPictureEnabled && this.inited && !this.isMobile
    },
    // 音量控制是否开启
    volumeEnabled () {
  ...
  mounted () {
    ...
    videoWrap.addEventListener('enterpictureinpicture', this.pictureInPictureChange)
    videoWrap.addEventListener('leavepictureinpicture', this.pictureInPictureChange)
  ...
  beforeDestroy () {
    ...
    const videoWrap = this.$refs.videoWrap
    videoWrap.removeEventListener('enterpictureinpicture', this.pictureInPictureChange)
    videoWrap.removeEventListener('leavepictureinpicture', this.pictureInPictureChange)
  ...
  methods: {
    ...
    switchQuality (item) {
      ...
      if (this.config.isEncrypt) {
        ...
        const src = this.qualityList.find(quality => quality.type === item.type).url
    ...
    dragMove (e) {
      ...
      if (this.player.video.duration) {
        ...
        const time = this.player.video.duration * (tx / progressBarWrap.offsetWidth)
    ...
    volumeBarClick (e) {
      ...
      const volume = 1 - (e.clientY - volumeBarWrap.getBoundingClientRect().top) / volumeBarWrap.clientHeight
      this.setVolume(volume)
    },
    getVolume () {
      const volume = localStorage.getItem('GkPlayerVolume')
      return (volume >= 0 && volume <= 1) ? +volume : this.player.video.volume
    },
    setVolume (volume) {
    ...
    videoTouchSeekMove (e) {
      const moveX = (e.clientX || e.changedTouches[0].clientX) - this.videoTouchStartX
      const moveY = (e.clientY || e.changedTouches[0].clientY) - this.videoTouchStartY
      // 只有左右滑动时处理
      if (moveX * moveX - moveY * moveY > 10) {
        ...
        if (this.player.duration < 300) {
          ...
          percentage += moveX / this.$refs.videoWrap.clientWidth * 300 / this.player.duration
        }
        percentage = Math.max(percentage, 0)
        percentage = Math.min(percentage, 1)
        this.thumbPercentage = percentage * 100 + '%'
     ...
     */
    showOutlineTip (data) {
      this.$emit('toggleOutLineTip', data)
    },
    hideOutlineTip () {
