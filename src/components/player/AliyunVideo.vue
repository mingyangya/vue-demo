    ...
    exportPlayer (player) {
      ...
      this.player = {
        ...
        switchQuality (key) {
          ...
          // 1. 初始化播放器时，取自动播放状态
          // 2. 播放中时，取当前播放状态
          const isPlay = that.initSwitch ? that.config.autoPlay : !player.paused()
          if (that.initSwitch) { that.initSwitch = false }

          const url = player._urls.find(url => url.definition.toLowerCase() === key.toLowerCase())
