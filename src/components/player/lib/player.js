...
class Player {
  ...
  initMSE (src) {
     ...
     */
    if (type === 'hls' && (this.video.canPlayType('application/x-mpegURL') || this.video.canPlayType('application/vnd.apple.mpegURL'))) {
      type = 'normal'
    }
  ...
  initVideo (src) {
    ...
    this.on('progress', () => {
      const percentage = this.video.buffered.length
        ? this.video.buffered.end(this.video.buffered.length - 1) / this.video.duration
        : 0
