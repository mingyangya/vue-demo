      ...
      <div class="video-point-container" v-if="showVideoPoint">
        <div :class="['ul-wrap', {'ul-wrap-min': !showSwitchBtn}]">
          ...
          <ul class="list video-point-list" @scroll="scroll" ref="ulEle" :style="ulStyle" >
            <li v-for="(item, i) in points" :key="i" @click.stop="clickItem(item)" :class="{ 'paused' : paused, 'active': (currentT >= item.period[0]) && (currentT < item.period[1])}" :style="{'width': liWidth + 'px' }">{{item.seconds | formatSecond}} {{item.desc}}</li>
          </ul>
...
export default {
  ...
  computed: {
    ...
    endTime () {
      return this.duration || Number.MAX_VALUE
    },
    ulStyle () {
  ...
  methods: {
    ...
    clickItem ({ seconds }) {
      ...
      this.$emit('click-video-point', seconds)
